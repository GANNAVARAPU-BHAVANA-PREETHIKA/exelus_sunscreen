...
export const Cart = () => {
  ...
  return (
    <>
      ...
      >
        {/* Header */}
        <div className="p-8 border-b border-brand-paper/5 flex items-center justify-between">
          <div className="flex items-center space-x-4">
            <ShoppingBag size={24} className="text-brand-olive" />
            <h2 className="text-2xl font-serif text-brand-paper">Your Cart</h2>
            <span className="bg-brand-olive/10 text-brand-olive text-[10px] px-2 py-1 rounded-full font-bold uppercase tracking-widest">
              {totalItems} Items
            </span>
          </div>
          <button
        ...
        <div className="flex-grow overflow-y-auto p-8 space-y-8 custom-scrollbar">
          {cart.length === 0 ? (
            ...
            cart.map((item) => (
              ...
              >
                <div className="w-24 h-24 rounded-2xl overflow-hidden bg-brand-surface border border-brand-paper/5 flex-shrink-0">
                  <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                </div>
                <div className="flex-grow">
                  <div className="flex justify-between items-start mb-2">
                    <h4 className="font-serif text-brand-paper leading-tight">{item.name}</h4>
                    <button
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-4 bg-brand-paper/5 rounded-full px-3 py-1">
                      <button
                    ...
                    <span className="font-bold text-brand-paper">₹{(item.price * item.quantity).toLocaleString()}</span>
                  </div>
                </div>
              </div>
            ))
        ...
        {/* Footer */}
        {cart.length > 0 && (
          <div className="p-8 border-t border-brand-paper/5 bg-brand-surface/50 backdrop-blur-md">
            ...
            <button className="w-full bg-brand-olive text-brand-ink py-5 rounded-2xl text-xs uppercase tracking-[0.3em] font-black shadow-xl shadow-brand-olive/10 hover:bg-white transition-colors">
              Checkout Now
            </button>
